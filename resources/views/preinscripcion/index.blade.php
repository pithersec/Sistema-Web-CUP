    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logo {
            ...
            font-family: 'Merriweather', serif; font-size: 11px; font-weight: 700; color: white;
        }

        .topbar h1 { font-family: 'Merriweather', serif; color: white; font-size: 16px; }
        ...
        .steps {
            ...
            border-radius: 10px; padding: 20px 28px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); margin-bottom: 24px;
        }
        ...
        .step.active .step-circle { background: var(--azul-claro); color: white; }
        .step.pending .step-circle { background: var(--gris-claro); color: var(--gris); }
        .step-label { font-size: 12px; font-weight: 600; }
        .step.active .step-label { color: var(--azul); }
        .step.pending .step-label { color: var(--gris); }
        .step-line { flex: 1; height: 2px; background: var(--gris-claro); margin: 0 8px; }
        ...
        .card-header {
            background: var(--azul); padding: 16px 24px;
        ...
        label {
            ...
            color: var(--azul); text-transform: uppercase; letter-spacing: 0.7px; margin-bottom: 6px;
        }

        .req { color: var(--rojo); font-weight: 700; }

        input, select {
            width: 100%; padding: 10px 14px; border: 1.5px solid var(--gris-claro);
            border-radius: 6px; font-family: 'Source Sans 3', sans-serif;
            font-size: 14px; color: #333; background: white; outline: none;
        }

        input:focus, select:focus { border-color: var(--celeste); }
        .has-error { border-color: var(--rojo) !important; background: #fdf0f0; }

        .field-error { color: var(--rojo); font-size: 12px; margin-top: 4px; }

        .section-divider {
            margin: 20px 0 16px; border-top: 1px solid var(--gris-claro); padding-top: 16px;
        }

        .section-divider h3 {
            font-size: 13px; font-weight: 600; color: var(--azul-claro);
            text-transform: uppercase; letter-spacing: 0.5px;
        }

        .footer-buttons {
            display: flex; justify-content: flex-end; gap: 12px;
            margin-top: 24px; padding-top: 20px; border-top: 1px solid var(--gris-claro);
        }

        .btn-secondary {
            padding: 11px 24px; border: 1.5px solid var(--gris-claro); border-radius: 6px;
            background: white; color: var(--gris); font-family: 'Source Sans 3', sans-serif;
            font-size: 14px; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-block;
        }

        .btn-primary {
            padding: 11px 28px; border: none; border-radius: 6px; background: var(--azul);
            color: white; font-family: 'Source Sans 3', sans-serif; font-size: 14px;
        ...
        .alert-danger {
            background: #fadbd8; color: #78281f; padding: 14px 18px; border-radius: 8px;
            margin-bottom: 16px; font-size: 14px; border-left: 4px solid var(--rojo);
        }
        ...
        .success-box {
            ...
            margin-bottom: 16px; font-size: 14px; border-left: 4px solid #10b981;
        }
    </style>
</head>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('preinscripcion.registrar') }}" method="POST">
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <select name="codigo_carrera1" class="{{ $errors->has('codigo_carrera1') ? 'has-error' : '' }}" required>
                                ...
                                <option value="{{ $carrera->codigo }}" {{ old('codigo_carrera1') == $carrera->codigo ? 'selected' : '' }}>{{ $carrera->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="codigo_carrera2" class="{{ $errors->has('codigo_carrera2') ? 'has-error' : '' }}" required>
                                <option value="">-- Selecciona una carrera --</option>
                                @foreach($carreras as $carrera)
                                <option value="{{ $carrera->codigo }}" {{ old('codigo_carrera2') == $carrera->codigo ? 'selected' : '' }}>{{ $carrera->nombre }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="footer-buttons">
                        <a href="{{ route('login') }}" class="btn-secondary">Volver al Login</a>
